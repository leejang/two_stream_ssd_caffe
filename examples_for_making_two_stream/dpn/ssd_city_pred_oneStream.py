...
if __name__ == '__main__':
    ...
    test_result_dir = wk_root + '/results/cityscape/test_oneStream_%ds' % (future,)
    ...
    model_def = model_root + '/cityscape/SSD_oneStream_500x500/deploy.prototxt'
    model_weights = model_root + '/cityscape/SSD_oneStream_500x500/cityscape_SSD_oneStream_500x500_iter_10000.caffemodel'
    assert os.path.isfile(model_def)
    assert os.path.isfile(model_weights)

    reg_model_root = model_root + '/cityscape/SSD_regressOneStream_%ds_25x25' % (future,)
    reg_model_def = reg_model_root + '/deploy_%s.prototxt' % (suffix,)
    reg_model_weights = reg_model_root + '/cityscape_SSD_regressOneStream_%ds_25x25_%s_iter_20000.caffemodel' % (future,suffix)
