...
if __name__ == '__main__':
    ...
    test_result_dir = wk_root + '/results/adl/test_oneStream_%ds' % (future,)
    ...
    model_def = model_root + '/adl/SSD_oneStream_500x500/deploy.prototxt'
    model_weights = model_root + '/adl/SSD_oneStream_500x500/adl_SSD_oneStream_500x500_iter_32293.caffemodel'
    assert os.path.isfile(model_def)
    assert os.path.isfile(model_weights)

    reg_model_root = model_root + '/adl/SSD_regressOneStream_%ds_25x25' % (future,)
    reg_model_def = reg_model_root + '/deploy_%s.prototxt' % (suffix,)
    reg_model_weights = reg_model_root + '/adl_SSD_regressOneStream_%ds_25x25_%s_iter_30000.caffemodel' % (future,suffix)
