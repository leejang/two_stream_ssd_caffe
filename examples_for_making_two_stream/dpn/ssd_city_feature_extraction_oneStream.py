...
model_dir = wk_dir + '/models/cityscape/SSD_oneStream_500x500'
...
model_weights = model_dir + '/cityscape_SSD_oneStream_500x500_iter_30000.caffemodel'
...
for dname in sorted(data_dict.keys()):
    ...
    for fns in sorted(data_dict[dname].keys()):
        ...
        if fns+future in data_dict[dname].keys():
            fnos.append(fns)
