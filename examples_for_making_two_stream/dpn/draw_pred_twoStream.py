...
if __name__ == '__main__':
    ...
    test_result_dir = wk_root + '/results/adl/test_twoStream_%ds' % (future,)
    ...
    model_def = model_root + '/adl/SSD_twoStream_500x500/deploy.prototxt'
    model_weights = model_root + '/adl/SSD_twoStream_500x500/adl_SSD_twoStream_500x500_iter_38209.caffemodel'
    assert os.path.isfile(model_def)
    assert os.path.isfile(model_weights)

    reg_model_root = model_root + '/adl/SSD_regressTwoStream_%ds_25x25' % (future,)
    reg_model_def = reg_model_root + '/deploy_%s.prototxt' % (suffix,)
    reg_model_weights = reg_model_root + '/adl_SSD_regressTwoStream_%ds_25x25_%s_iter_%d.caffemodel' % (future,suffix,max_iter)
    ...
    for ix,line in enumerate(useFiles):
        ...
        fid = int(fname[6:])
        future_img = 'frame_%05d.jpg' % (fid+30*future)
