...
hdf_f.create_dataset('data', (num_of_images - 5,256,25,25), dtype='f')
hdf_f.create_dataset('label', (num_of_images - 5,256,25,25), dtype='f')
...
for test_img in sorted(glob.glob(images), key=key_func):
    ...
    if base_name.isdigit():
      if idx_cnt < (num_of_images - 5):
        ...
        read_label = hdf_raw_f.get('label')[idx_cnt + 5]
